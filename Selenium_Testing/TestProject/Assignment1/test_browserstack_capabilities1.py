...
device_menu = web_driver.find_element_by_id("doc-device-options-wrapper")
# Scroll until the element is found
web_driver.execute_script("arguments[0].scrollIntoView();", device_menu)
# If its a table we need to scroll else we can live without scroll
table_menu = web_driver.find_element_by_class_name("doc-device.doc-device-Tablet")
# Scroll until the element is found
web_driver.execute_script("arguments[0].scrollIntoView();", table_menu)
# Select the device
device = web_driver.find_element_by_xpath("//*[@id='doc-device-options']/div[2]/div[2]/ul/li[2]/a")
web_driver.execute_script("arguments[0].scrollIntoView();", table_menu)
device.click()
